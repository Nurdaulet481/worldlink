...
const controlsTimeoutMap = new Map(); // Карта для автоскрытия контролов через таймаут
...
// Инициализация Observer для автовоспроизведения видео при скролле
function initVideoObserver() {
    const options = {
        root: null,
        rootMargin: '0px',
        threshold: 0.6 // Видео должно быть видно хотя бы на 60%
    };
    ...
    videoObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                // Если пользователь уже разблокировал звук в этой сессии — воспроизводим СО ЗВУКОМ!
                if (userAudioEnabled) {
                    video.muted = false;
                    video.volume = 1;
                    if (muteIcon) {
                    ...
                    // До первого клика — запускаем без звука (чтобы браузер не заблокировал автоплей)
                    video.muted = true;
                    if (muteIcon) {
                ...
                video.play().then(() => {
                    ...
                    // Если браузер всё же отклонил воспроизведение со звуком, падаем до mute
                    console.warn(`Автовоспроизведение видео ${postId} заблокировано со звуком, переключаем на muted:`, err);
                    video.muted = true;
                    if (muteIcon) {
                        muteIcon.classList.remove('fa-volume-high');
                        muteIcon.classList.add('fa-volume-xmark');
                ...
                // Ставим на паузу при уходе из зоны видимости
                video.pause();
                if (playIcon) {
                ...
                // При уходе из кадра скрываем контролы
...
async function loadFeedPosts() {
    ...
    try {
        ...
        if (posts.length === 0) {
            feedContainer.innerHTML = '<div class="empty-feed">Пока нет ни одного поста. Опубликуйте первый!</div>';
            return;
        }
        ...
        posts.forEach(post => {
            ...
            // Регистрируем видео в наблюдателе
        ...
        // Прокрутка к целевому посту, если перешли по ссылке из профиля
        scrollToTargetPost();

    } catch (error) {
        console.error('Ошибка при загрузке постов:', error);
        feedContainer.innerHTML = '<div class="empty-feed">Ошибка загрузки публикаций</div>';
    }
}

// Прокрутка и подсветка выбранного поста из профиля
    ...
    if (postId) {
        setTimeout(() => {
            ...
            if (targetPost) {
                ...
                targetPost.style.transition = 'box-shadow 0.3s ease, border-color 0.3s ease';
                targetPost.style.boxShadow = '0 0 20px rgba(0, 136, 204, 0.5)';
                setTimeout(() => {
...
function createPostCard(post) {
    ...
    if (!post.is_self) {
        const btnText = post.is_subscribed ? 'Подписки' : 'Подписаться';
        const btnClass = post.is_subscribed ? 'btn-subscribe subscribed' : 'btn-subscribe';
        subscribeBtnHtml = `<button class="${btnClass}" onclick="toggleSubscribe(${post.user_id}, this)">${btnText}</button>`;
    }

    // Кастомный медиаплеер без стандартных контролов браузера
    let mediaHtml = '';
    if (post.media_path && post.media_path.trim() !== '') {
        const isVideo = post.media_path.match(/\.(mp4|webm|ogg)$/i);
        const fullMediaPath = post.media_path.startsWith('/') || post.media_path.startsWith('http')
            ? post.media_path
            : `/static/${post.media_path}`;

        if (isVideo) {
            mediaHtml = `
                <div class="post-media custom-video-container" id="video-container-${post.id}">
                    <video 
                        src="${fullMediaPath}" 
                        loop 
                        playsinline 
                    ...
                    ></video>
                    <!-- Оверлей управления: по умолчанию скрыт через CSS -->
                    <div class="custom-media-controls">
                        ...
                        <div class="volume-control-wrapper">
                            <button class="custom-mute-btn" onclick="toggleVideoMute(${post.id})" title="Звук">
                                <i class="fa-solid fa-volume-xmark" id="mute-icon-${post.id}"></i>
                            </button>
                            <input 
                                type="range" 
                                class="volume-slider" 
            ...
            mediaHtml = `<div class="post-media"><img src="${fullMediaPath}" alt="Пост"></div>`;
        }
    }

    // Аватар автора
    const avatarHtml = formatAvatarHtml(post.author_avatar, post.author_name);
    ...
    card.innerHTML = `
        <div class="post-header">
            <a href="/profile/${post.user_id}" class="author-info" title="Перейти в профиль">
        ...
        <div class="post-actions">
            ...
            ${!post.is_self ? `
                <a href="/messages/${post.user_id}" class="action-btn message-btn" title="Написать автору">
...
// Обработка клика по самому видео (включает звук + управляет паузой/воспроизведением)
function handleVideoClick(postId) {
    ...
    if (video) {
        // Если видео было без звука или звук ещё не включался глобально — ВКЛЮЧАЕМ ЕГО!
        if (video.muted || !userAudioEnabled) {
            ...
            // Если звук уже работает — клик работает как Пауза / Плей
...
// Показать контролы и плавно скрыть их через 3 секунды
function showControlsTemporarily(postId) {
    ...
    if (controlsTimeoutMap.has(postId)) {
        clearTimeout(controlsTimeoutMap.get(postId));
    }
    ...
    controlsTimeoutMap.set(postId, timeout);
}
...
function toggleVideoMute(postId) {
    ...
    userAudioEnabled = !video.muted; // Запоминаем выбор пользователя
...
// Изменение уровня громкости через ползунок
...
// Вычисление цвета аватара без фото по имени
...
// Форматирование аватарок
function formatAvatarHtml(avatarPath, name) {
    const cleanName = (name || '').trim();
    const firstLetter = cleanName ? cleanName[0].toUpperCase() : '?';

    if (avatarPath && avatarPath.trim() !== '' && !avatarPath.includes('undefined') && !avatarPath.includes('null')) {
        const src = avatarPath.startsWith('http') || avatarPath.startsWith('/')
            ? avatarPath
            : `/static/${avatarPath}`;
    ...
    const bgColor = getAvatarBgColor(cleanName);
    return `<div class="avatar-placeholder" style="background-color: ${bgColor};">${firstLetter}</div>`;
}

// Лайки с сохранением на сервере
...
async function loadComments(postId, postCard) {
    ...
    try {
        ...
        comments.forEach(c => {
            ...
            const avatarHtml = formatAvatarHtml(c.user_avatar, c.user_name);
...
// Подписки
async function toggleSubscribe(userId, buttonElement) {
    try {
        ...
        if (result.status === 'success') {
            if (result.is_subscribed) {
                buttonElement.innerText = 'Подписки';
...
// Закрытие комментариев при клике вне карточки
